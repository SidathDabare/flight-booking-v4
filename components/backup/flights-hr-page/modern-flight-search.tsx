...
const hasStoredLocationCodes = (): { origin: boolean; destination: boolean } => {
  try {
...
const createFallbackAirportFromCode = async (iataCode: string): Promise<any | null> => {
  try {
    ...
    console.error("Failed to fetch airport data for IATA code:", iataCode, error);
    return null;
  }
};
...
export default function ModernFlightSearch({
  ...
  const [shouldFetchData, setShouldFetchData] = useState<{ origin: boolean; destination: boolean }>({ origin: false, destination: false });
  ...
  useEffect(() => {
    ...
    console.log("Initial localStorage check:", {
      ...
      hasStoredData,
      loadingTime,
      savedData: saved ? { origin: saved.originLocationCode, dest: saved.destinationLocationCode } : null
    });
  ...
  useEffect(() => {
    const loadAndRestoreData = async () => {
      ...
      if (saved?.originLocationCode && !airportData?.originAirport) {
        console.log("🔄 Fetching missing origin airport data for:", saved.originLocationCode);
        const originAirport = await createFallbackAirportFromCode(saved.originLocationCode);
        if (originAirport) {
      ...
      if (saved?.destinationLocationCode && !airportData?.destinationAirport) {
        console.log("🔄 Fetching missing destination airport data for:", saved.destinationLocationCode);
        const destinationAirport = await createFallbackAirportFromCode(saved.destinationLocationCode);
        if (destinationAirport) {
          updatedAirportData.destinationAirport = destinationAirport;
          console.log("✅ Created destination airport object:", destinationAirport);
        }
      }
      ...
      if (saved) {
        ...
        if (saved.originLocationCode) {
          console.log("🎯 Setting origin location code:", saved.originLocationCode);
        ...
        if (saved.destinationLocationCode) {
          form.setValue("destinationLocationCode", saved.destinationLocationCode, {
            shouldValidate: false,
            shouldDirty: false,
          });
        }
                                ...
                                <div className="relative w-full">
                                  <AirportCombobox
                                    ...
                                    getAirportValue={(value: string) => {
                                      ...
                                      const currentValue = form.getValues("originLocationCode");
                                      if (value === "" && currentValue && isLoading) {
                              ...
                              <FormControl>
                                <div className="relative w-full">
                                  <AirportCombobox
                                    ...
                                    getAirportValue={(value: string) => {
                                      // Don't override localStorage data with empty values during loading
                                      const currentValue = form.getValues("destinationLocationCode");
                                      if (value === "" && currentValue && isLoading) {
                                        // Component initializing, don't override saved data
                                        return;
                                      }
                    ...
                    <div className="w-full lg:w-2/6 flex flex-col sm:flex-row items-stretch">
                      ...
                      <div className="px-2 py-2 flex-1">
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button
                              variant="ghost"
                              className="w-full h-[57px] border-gray-500/50 px-3 text-sm font-medium border rounded-none hover:bg-gray-50 text-left justify-start"
                            >
                              <span className="flex items-center gap-2">
                                ...
                                <span className="truncate">
                                  ...
                                  <span className="xs:hidden">Pax</span>
                                </span>
                              </span>
                            </Button>
                          </PopoverTrigger>
                      ...
                      <div className="px-2 py-2 flex-1">
                        <Button
                          ...
                          className={`h-[57px] w-full text-white font-medium rounded-full transition-all duration-200 ${
                            isDisabled
