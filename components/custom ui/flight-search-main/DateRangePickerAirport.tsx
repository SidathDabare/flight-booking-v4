...
export default function DateRangePickerAirport({
  departureDate,
  returnDate,
  className,
  getDateValue,
  ...
  const t = useTranslations("flightSearch.datePicker");
        ...
        <div className="w-full p-6">
          ...
          <div className="grid grid-cols-7 mb-1">
            {dayNames.map((day) => (
              <div
                key={day}
                className="h-10 w-10 flex items-center justify-center text-xs font-medium text-gray-700 dark:text-gray-300"
              >
                {day}
              </div>
            ))}
          ...
          <div className="grid grid-cols-7 gap-0 text-gray-600 dark:text-gray-400">
            {days.map((day, index) => {
              if (!day) {
              ...
              return (
                <Button
                  ...
                  disabled={isDisabled}
                  className={cn(
                    "h-10 w-10 text-xs font-normal flex items-center justify-center transition-colors relative p-0",
                    "hover:text-gray-900",
                    {
                      "text-gray-400 cursor-not-allowed": isDisabled,
                      "bg-gray-800 text-gray-100 hover:bg-gray-900 hover:text-gray-100 rounded-none":
                        isSelected,
                      "bg-gray-300 text-gray-700": isInRange && !isSelected,
                      "text-blue-500 hover:text-blue-600 font-bold rounded-none":
                        isToday && !isSelected,
                      "rounded-l-sm": isStart && twoWay,
                      "rounded-r-sm": isEnd && twoWay,
                      "rounded-sm": !twoWay || (!isInRange && !isSelected),
                    }
                  )}
                >
                  {day.getDate()}
  ...
  return (
    <div ref={containerRef} className={cn("w-full relative", className)}>
      <div
        ...
        tabIndex={0}
        className="w-full h-[57px] px-3 py-3 border border-gray-300 cursor-pointer hover:border-blue-600 focus:border-blue-500 focus:outline-none transition-colors"
      >
        <div className="flex items-center justify-between h-full relative">
          <div className="flex-1 w-[80%]">
            {formatDisplayValue() ? (
              ...
              <span className="text-sm">{t("pickDate")}</span>
            )}
          </div>
          <CalendarIcon className="h-4 w-4 absolute right-1" />
        </div>
      ...
      {isOpen && (
        <div
          className={cn(
            "absolute left-0 bg-white dark:bg-gray-900 rounded-none shadow-xl border border-gray-200 dark:border-gray-700 z-60 w-full md:w-[300px] max-h-96 overflow-auto",
        ...
        >
          <div className="relative">
            <Button
              ...
              className="absolute left-2 top-6 z-10 h-8 w-8 border text-gray-600 hover:text-gray-800 border-gray-400 bg-white hover:bg-gray-50 flex items-center justify-center p-0"
            >
            ...
            <Button
              ...
              className="absolute right-2 top-6 z-10 h-8 w-8 border text-gray-600 hover:text-gray-800 border-gray-400 bg-white hover:bg-gray-50 flex items-center justify-center p-0"
            >
